...
def generar_report_mail(informe_data):
    ...
    contenido.append(
        Paragraph(
            '<font size="24" color="DarkBlue">Informe de Evaluación OSINT</font>',
    ...
    for item in informe_data:
        ...
        contenido.append(
            Paragraph(
                "<b>Nivel de Criticidad:</b> {}".format(item["Nivel de Criticidad"]),
                parrafo_style,
            )
        )
        ...
        for recomendacion_info in recomendaciones:
            ...
            contenido.append(
                Paragraph(
                    f'&nbsp;&nbsp;&nbsp;&nbsp;Nivel de Impacto: {recomendacion_info["impacto"]}',
                    parrafo_style,
                )
            )
...
def generar_report_username(datos_importantes):
    ...
    for red_social, detalles in datos_importantes.items():
        ...
        contenido.append(
            Paragraph(f'<b>URL del Usuario:</b> {detalles["url_user"]}', parrafo_style)
        )

        # Nivel Crítico
        contenido.append(
            Paragraph(
                f'<b>Nivel Crítico:</b> {detalles["nivel_critico"]}', parrafo_style
            )
        )
...
def generar_report_domain(data, domain):
    ...
    contenido.append(
        Paragraph("<b>Dominio:</b> {}".format(data["domain"]), parrafo_style)
    )
    contenido.append(
        Paragraph(
            "<b>Nombre de la Organización:</b> {}".format(data["organization"]),
            parrafo_style,
        )
    )
    contenido.append(
    ...
    contenido.append(
        Paragraph("<b>Descripción:</b> {}".format(data["description"]), parrafo_style)
    )
    contenido.append(
        Paragraph("<b>Industria:</b> {}".format(data["industry"]), parrafo_style)
    )
    contenido.append(
        Paragraph("<b>País:</b> {}".format(data["country"]), parrafo_style)
    )
    contenido.append(Paragraph("<b>Ciudad:</b> {}".format(data["city"]), parrafo_style))
    contenido.append(
        Paragraph("<b>Código Postal:</b> {}".format(data["postal_code"]), parrafo_style)
    )
    contenido.append(
        Paragraph("<b>Dirección:</b> {}".format(data["street"]), parrafo_style)
    )
    ...
    for email_info in data["emails"]:
        ...
        contenido.append(
            Paragraph(
                "<b>Confianza:</b> {}%".format(email_info["confidence"]), parrafo_style
            )
        )
        contenido.append(
            Paragraph(
                "<b>Origen:</b> {}".format(email_info["sources"][0]["domain"]),
                parrafo_style,
            )
        )
        ...
        if email_info.get("phone_number"):
            contenido.append(
                Paragraph(
                    "<b>Teléfono:</b> {}".format(email_info["phone_number"]),
                    parrafo_style,
                )
            )
        contenido.append(Spacer(1, 10))  # Aumentar el espacio vertical
    ...
    contenido.append(
        Paragraph(
            "<b>Enlaces a Redes Sociales de la Organización:</b>", subtitulo_style
        )
    )
    contenido.append(Spacer(1, 10))  # Aumentar el espacio vertical
    if data["twitter"]:
    ...
    recomendaciones_generales = [
        {
            "recomendacion": "Verificación de Datos: Siempre verifica la precisión de la información antes de utilizarla para tomar decisiones importantes. Los datos pueden volverse obsoletos o inexactos con el tiempo.",
            "impacto": "Medio",
        },
        {
            "recomendacion": "Uso Responsable: Utiliza la información de manera ética y legal. No la utilices para fines maliciosos, como el phishing o la suplantación de identidad.",
            "impacto": "Alto",
        },
        {
            "recomendacion": "Seguridad de Datos: Si manejas datos personales, asegúrate de seguir las mejores prácticas de seguridad de datos para proteger la información de posibles brechas de seguridad.",
            "impacto": "Alto",
        },
    ]
    ...
    for recomendacion_info in recomendaciones_generales:
        ...
        impacto = recomendacion_info["impacto"]
        contenido.append(
            Paragraph(f"{recomendacion} (Impacto: {impacto})", parrafo_style)
        )
        contenido.append(Spacer(1, 10))  # Aumentar el espacio vertical
    ...
    recomendaciones_especificas = [
        {
            "recomendacion": "Redes Sociales: Revisa la configuración de privacidad en tus cuentas de redes sociales y ajusta la visibilidad de tus perfiles y publicaciones según tu preferencia.",
            "impacto": "Medio",
        },
        {
            "recomendacion": "Contraseñas Seguras: Utiliza contraseñas seguras y cámbialas regularmente. Habilita la autenticación de dos factores (2FA) cuando esté disponible para mejorar la seguridad de tus cuentas.",
            "impacto": "Alto",
        },
        {
            "recomendacion": "Compartir Datos Personales: Evita compartir información personal o sensible en tus publicaciones en línea. Protege tu privacidad.",
            "impacto": "Medio",
        },
        {
            "recomendacion": "Aplicaciones de Terceros: Ten cuidado al autorizar aplicaciones de terceros en tus cuentas de redes sociales. Verifica su reputación antes de conceder acceso.",
            "impacto": "Medio",
        },
        {
            "recomendacion": "Actividad de Cuenta: Revisa periódicamente la actividad en tus cuentas y ajusta la configuración de notificaciones para un mayor control.",
            "impacto": "Bajo",
        },
        {
            "recomendacion": "Mensajes Sospechosos: Sé escéptico ante los mensajes sospechosos que recibas y no hagas clic en enlaces no verificados.",
            "impacto": "Alto",
        },
    ]
    ...
    if theharvester_data:
        ...
        contenido.append(
            Paragraph(
                "No se encontró información de TheHarvester para el dominio proporcionado.",
                parrafo_style,
            )
        )
    ...
    recomendaciones_theharvester = [
        {
            "recomendacion": "ASNs: Investiga y verifica la seguridad de los ASNs asociados a tu dominio.",
            "impacto": "Alto",
        },
        {
            "recomendacion": "Emails: Asegúrate de que los correos electrónicos asociados a tu dominio sean seguros y estén protegidos contra phishing.",
            "impacto": "Alto",
        },
        {
            "recomendacion": "Hosts: Realiza un análisis de seguridad en los hosts utilizados por tu dominio para detectar posibles vulnerabilidades.",
            "impacto": "Medio",
        },
        {
            "recomendacion": "URLs Interesantes: Verifica la seguridad de las URLs que has identificado como interesantes para tu dominio.",
            "impacto": "Medio",
        },
        {
            "recomendacion": "IPs: Asegúrate de que las IPs asociadas a tu dominio estén bien configuradas y protegidas contra ataques.",
            "impacto": "Alto",
        },
        {
            "recomendacion": "Shodan: Monitorea regularmente Shodan para asegurarte de que no haya información sensible expuesta públicamente.",
            "impacto": "Bajo",
        },
    ]
